...
async def list_diary(
    ...
    - days=0 表示不限时间范围，全部记录分页查询（翻页可查历史）
    - days>0 仅查最近 N 天
    - search 按 note / mood_label 模糊匹配，分页同样生效
    返回 {code, data: {items, total, page, page_size, pages, has_more}}
    """
    q = select(MoodEntry).where(MoodEntry.user_id == current_user["user_id"])
    if days and days > 0:
        since = datetime.now(CN_TZ) - timedelta(days=days)
        q = q.where(MoodEntry.created_at >= since)
    if search:
        pattern = f"%{search}%"
        q = q.where(

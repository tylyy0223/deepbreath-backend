...
async def practice_stats(
    ...
    时间边界统一按北京时间（UTC+8）自然周（周一 00:00 起）；
    日期切分用 SQL 内联 AT TIME ZONE，不依赖连接时区。
    """
    user_id = current_user["user_id"]
    CN_TZ = timezone(timedelta(hours=8))
    now_cn = datetime.now(CN_TZ)
    # 本周一 00:00（北京时间）
    monday = now_cn.date() - timedelta(days=now_cn.weekday())
    week_start = datetime(monday.year, monday.month, monday.day, tzinfo=CN_TZ)

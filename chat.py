...
async def chat_send(req: ChatRequest, current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    async def event_stream():
        ...
        try:
            ...
            # RAG 并行检索（识别到书目时跳过通用检索）
            rag_task = None
            if req.use_rag and not book:
                async def _rag():
                    try:
                        results = await asyncio.to_thread(search_wiki, req.message, top_k=5 if req.mode != "reading" else 15)
                        if results:
            ...
            # Redis 缓存检查（非 RAG 模式）
            cache_key = None
            if not req.use_rag and len(req.message) >= 4:
